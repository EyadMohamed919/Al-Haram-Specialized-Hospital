...
function AddNewdonationRecord() {
    ...
    if (validateForm(id, name, BloodType, date, country)) {
        const table = document.getElementById("Donationtable");
        const row = table.insertRow(-1);

        row.insertCell(0).innerText = id;
        row.insertCell(1).innerText = name;
        row.insertCell(2).innerText = BloodType;
...
function UpdateDonationRecord() {
    ...
    if (!validateForm(id, name, BloodType, date, country)) return;
    ...
    for (let i = 1; i < table.rows.length; i++) {
        if (table.rows[i].cells[0].innerText === id) {
            ...
            table.rows[i].cells[2].innerText = BloodType;
